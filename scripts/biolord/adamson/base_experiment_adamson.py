...
adata = sc.read(
    DATA_DIR + "perturbations/adamson/adamson_biolord.h5ad",
)

adata_single = sc.read(
    DATA_DIR + "perturbations/adamson/adamson_single_biolord.h5ad",
)

with open(DATA_DIR + "perturbations/adamson/no_perturb_subgroup_analysis.pkl", "rb") as f:
...
def train_dataset():
    ...
    parser.add_argument("--project_name", type=str, default="adamson") 
    ...
    if args.optimal_params:
        ...
        from adamson_config_optimal import varying_arg
    else:
        print("Using random params for train.")
        from adamson_config import varying_arg
    ...
    if args.optimal_params:
        pd.DataFrame(res_biolord_delta_normalized).to_csv(DATA_DIR + "perturbations/adamson/biolord_normalized_mse_de_seeds.csv")
            
    for ood_set in ["unseen_single"]:
        print(ood_set)
        test_metrics_biolord_normalized_list = []
        for split_seed in range(1,6): #range(1,6):
